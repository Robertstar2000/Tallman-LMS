...
const LearnerDashboardV2: React.FC<{ user: User; refreshUser: () => void }> = ({ user: initialUser, refreshUser }) => {
    ...
    useEffect(() => {
        const refreshData = async () => {
            try {
                ...
                console.error("Dashboard Sync Error:", sanitizedError);
            }
        };
        refreshData();
    ...
    return (
        <div className="space-y-16 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-32">
            <header className="flex flex-col md:flex-row md:items-end md:justify-between gap-6">
                <div>
                    <h1 className="text-5xl font-black text-slate-900 tracking-tight leading-none italic uppercase">Personnel Console <span className="text-xs text-indigo-500 not-italic tracking-normal bg-indigo-50 px-2 py-1 rounded-lg border border-indigo-200">v2.0</span></h1>
                    ...
                    {activeCourses.map((course, index) => (
                        ...
                        >
                            <div className="absolute inset-0">
                                <img
                                    src={course.thumbnail_url?.startsWith('data:') ? course.thumbnail_url : `${course.thumbnail_url}?v=${Date.now()}`}
                                    ...
                                    onError={(e) => {
                                        ...
                                        e.currentTarget.src = "https://images.unsplash.com/photo-1581091226825-a6a2a5aee158?auto=format&fit=crop&q=80&w=1000";
                                    }}
                                />
